...
-- Subjunção aditiva
constant subj_l_add {Δ : list Formula} {α β δ : Formula} :
	Proof (Sequent Δ α) → Proof (Sequent (β :: Δ) δ) → Proof (Sequent (Formula.subj α β :: Δ) δ)

-- Subjunção (com nominals, aditiva)
constant subj_l_n_add {Δ : list Formula} {α β δ : Formula} (X : Nominal) :
	Proof (Sequent Δ (Formula.elemOf X α)) → Proof (Sequent (Formula.elemOf X β :: Δ) δ) → Proof (Sequent (Formula.elemOf X (Formula.subj α β) :: Δ) δ)
